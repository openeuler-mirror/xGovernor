...
impl OperationPermissionControl for LocalOperationBackend {
    fn sandbox_denial_for_path(
        &self,
        path: &BackendPath,
        capability: operation_protocol::SandboxPermissionCapability,
        operation: &str,
    ) -> Result<Option<operation_protocol::SandboxPolicyDenial>, OperationError> {
...
mod tests {
    ...
    fn resolves_tilde_paths_against_home_dir() {
        ...
        let resolved = state
            .resolve_host_path("~/.xgovernor/tools/md_to_html.mjs", Path::new("/workspace"))
            .expect("resolve");

        assert_eq!(resolved, home.join(".xgovernor/tools/md_to_html.mjs"));
    }
