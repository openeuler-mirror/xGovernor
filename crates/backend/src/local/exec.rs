...
use crate::local::backend::LocalBackendState;
...
fn command_from_spec(
    ...
    policy: &crate::local::policy::LocalBackendPolicy,
...
mod linux_bubblewrap_tests {
    ...
    fn bubblewrap_exec_enforces_filesystem_policy() {
        ...
        let _guard = crate::process_group::process_group_test_lock()
            .lock()
            .unwrap();
        let root = super::test_workspace_root("xgovernor-bubblewrap-", "fs");
    ...
    fn bubblewrap_exec_can_unshare_network() {
        ...
        let _guard = crate::process_group::process_group_test_lock()
            .lock()
            .unwrap();
        let root = super::test_workspace_root("xgovernor-bubblewrap-", "net");
        let workspace = root.join("workspace");
        let writable = workspace.join("tmp");
        std::fs::create_dir_all(writable.as_path()).unwrap();
    ...
    fn bubblewrap_backend(
        ...
        writable: PathBuf,
        allow_network: bool,
    ) -> std::sync::Arc<dyn operation_protocol::OperationBackend> {
...
mod pipe_drain_tests {
    use crate::local::factory::local_backend;
    use operation_protocol::{capability::exec::ExecRequest, BackendPath};
    ...
    fn exec_returns_promptly_when_background_process_holds_stdout_pipe() {
        let _guard = crate::process_group::process_group_test_lock()
            .lock()
            .unwrap();
        let root = super::test_workspace_root("xgovernor-pipe-", "pipe");
        let workspace = root.join("workspace");
    ...
    fn exec_captures_background_output_when_writer_exits_within_grace() {
        let _guard = crate::process_group::process_group_test_lock()
            .lock()
            .unwrap();
        let root = super::test_workspace_root("xgovernor-pipe-", "bg");
        let workspace = root.join("workspace");
