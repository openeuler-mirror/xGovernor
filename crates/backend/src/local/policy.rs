...
use operation_protocol::{
    OperationError, SandboxPermissionCapability, SandboxPermissionGrantId,
        ...
        match kind {
            "macos_seatbelt" => {
                if !cfg!(target_os = "macos") {
                    return Err(LocalBuildError::Unsupported {
...
fn build_path_isolation_config(
    ...
    workspace_root: &Path,
    temp_root: &Path,
) -> Result<PathIsolationConfig, LocalBuildError> {
...
fn normalize_build_roots(
    field_name: &str,
    roots: Vec<String>,
) -> Result<Vec<PathBuf>, LocalBuildError> {
    roots
        .into_iter()
...
fn normalize_build_path(field_name: &str, path: &Path) -> Result<PathBuf, LocalBuildError> {
...
mod tests {
    ...
    fn sandbox_denial_for_read_path_requires_existing_host_path() {
        let root = std::env::temp_dir().join(format!(
            "xgovernor-local-policy-read-denial-{}",
            std::process::id()
        ));
    ...
    fn sandbox_denial_for_write_path_allows_missing_file_with_existing_parent() {
        let root = std::env::temp_dir().join(format!(
            "xgovernor-local-policy-write-denial-{}",
            std::process::id()
        ));
        let outside = root.join("outside");
        std::fs::create_dir_all(outside.as_path()).unwrap();
    ...
    fn sandbox_denial_for_path_skips_already_allowed_roots() {
        let root = std::env::temp_dir().join(format!(
            "xgovernor-local-policy-allowed-denial-{}",
            std::process::id()
        ));
    ...
    fn granted_write_path_allows_read_write_and_atomic_temp() {
        let root =
            std::env::temp_dir().join(format!("xgovernor-local-policy-rw-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(root.as_path());
    ...
    fn macos_seatbelt_is_unsupported_on_non_macos() {
        ...
        assert!(matches!(error, LocalBuildError::Unsupported { .. }));
    }
