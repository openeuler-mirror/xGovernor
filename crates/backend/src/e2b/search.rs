...
use super::backend::{shell_quote, E2bBackendState};
use super::exec::E2bExec;

pub(crate) struct E2bSearch {
    exec: E2bExec,
}
...
impl E2bSearch {
    pub(crate) fn new(state: Arc<E2bBackendState>) -> Self {
        Self {
            exec: E2bExec::new(state),
        }
    }
}

#[async_trait]
impl OperationSearch for E2bSearch {
    ...
    async fn grep(&self, request: GrepRequest) -> Result<GrepResult, OperationError> {
        ...
        let output = self
            .exec
            .run_shell_script(cmd.join(" ").as_str(), None)
            .await?;
