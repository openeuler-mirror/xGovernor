...
//! today that is `SessionApplication::check_lease_holder`, called from
//! `submit_turn`/`close`. (An earlier design gated this at a router's
//! `assert_lease_holder` check and a `SessionActor`'s `pending_turns` pop-time
//! check; neither exists in this crate.)
...
/// Daemon principals are meant to be cooperative background callers trusted
/// not to race user turns, distinguished from real `client_id`s by this
/// prefix rather than the old implicit `client_id = None` bypass (which was
/// unauditable — any caller who omitted `client_id` got the same silent
/// pass).
///
/// Carried over from the pre-split xiaoO design (see `[[xiaoo-two-repo-split]]`
/// in project memory) along with [`is_daemon_principal`] and the
/// `daemon_*_principal` builders below. Their only would-be callers —
/// `apps/server/src/cron/`, `httpserver/action_sink.rs`,
/// `httpserver/channel_ingress.rs` — import from the old `xiaoo_shared::gateway`
/// crate, not this one, and are not wired into `apps/server`'s module tree at
/// all (`httpserver/mod.rs`: "the former monolithic runtime router is no
/// longer part of the active workspace"). This crate's actual lease check
/// (`SessionApplication::check_lease_holder` /
/// `SessionLeaseTable::check_holder`) does not special-case this prefix
/// today — a `client_id` built with it is checked exactly like any other.
pub const DAEMON_PRINCIPAL_PREFIX: &str = "daemon:";

/// Whether `client_id` is a daemon-internal principal, by prefix only. Not
/// currently called by anything in this crate — see the caveat on
/// [`DAEMON_PRINCIPAL_PREFIX`]. If/when a real daemon-principal bypass is
/// wired into `check_lease_holder`, that bypass should emit its own
/// `tracing` event (mirroring `application::audit_log`'s target = "audit"
/// convention) rather than relying on this doc comment's word for it.
