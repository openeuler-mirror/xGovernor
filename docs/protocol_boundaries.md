...
4. **物理路径**：runtime 进程运行在 provider 提供的沙箱内；adapter 经由沙箱操作面（operation-protocol：exec / 端口 / 文件）到达它。adapter 依赖 provider/operation 合同，不反向可见。
   **已记录的例外（`apps/runtime-pi`，收窄版）**：`apps/runtime-pi` 组合 `InstanceManager`（每个 `backend_id` 一个，`local`/`e2b`），Pi 的工具*执行*经本地 HTTP 桥转发到真实 `Arc<dyn OperationBackend>`，这一部分与本条规则一致。控制面现已统一到 worker protocol：adapter 作为 supervisor，通过 `agent-runtime-protocol` 的 `WorkerRequest`/`WorkerResponse` NDJSON 驱动每 session 一个 `pi-worker`；worker 内部才处理 Pi 原生 JSON-RPC，并拉起 `pi --mode rpc`。worker/Pi 仍位于 daemon 本机，不经 operation-protocol，因为一次性全缓冲的 `OperationExec` 无法承接长生命周期双向 stdio；它们没有 provider ledger 或跨重启 reconcile，所附着的沙箱则仍由 `InstanceManager` 正常记账、恢复和配额控制。复原所需信息持久化在 §4 的状态检疫坑位，重启后由 `ensure_runtime_attached` 重放 `start(state=Some(..))`，重建 worker 和 Pi 进程并挂回原会话文件。这不改变第 5 条对称禁令：governor 编排代码仍只认 adapter/runtime protocol，不认 Pi 原生协议。
