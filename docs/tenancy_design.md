...
> 2026-08 后续更新：本节的启动加载 + SIGHUP 热重载已实现，但 schema 做了刻意收窄——只落地表格里当前真有强制点的字段（tokens、role、max_sessions、max_requests_per_minute），`allowed_workspace_kinds`/`git_host_allowlist`/`capability_ceiling`/`min_isolation_boundary`/`allowed_runtimes`/`max_concurrent_turns`/`max_sandboxes` 均**未**收进 `tenants.toml`——加一个没有消费者的 TOML 字段只是死配置，等某处代码真正读它时再补。实现：`apps/server/src/httpserver/tenant_config.rs`（schema/解析/校验，`#[serde(deny_unknown_fields)]`，拒绝重复 token/重复 tenant_id/空 tokens 列表）+ `apps/server/src/httpserver/auth.rs`（`TokenTable.entries` 改为 `Arc<ArcSwap<..>>`，新增 `reload()`，删除旧 `from_env()`/`TenantTokenEntry`）+ `apps/server/src/main.rs`（`XGOVERNOR_TENANTS_CONFIG_PATH`，默认 `$XGOVERNOR_DATA_DIR/tenants.toml`；`SIGHUP` 触发重载，reload 失败或产出空表**永不**应用，保留最后一份好的配置）。这是本文原定的"启动加载 + SIGHUP 热重载"目标的落地，取代了 §7 步骤 1/4 提到的 `XGOVERNOR_BEARER_TOKEN`/`XGOVERNOR_TENANT_TOKENS_JSON` 环境变量通道（已删除，无兼容路径）。
>
> 2026-08 再更新（租户自助查询 API，v1）：本节末句提到的"留给未来讨论"的自助查询/管理 API 现已落地一个刻意收窄的 v1——`GET /api/v1/sessions`（[http_api.md](./http_api.md) §3"会话列表 / 配额自助查询"）。范围明确排除：不含历史/已关闭会话（只返回 `opening/idle/running/paused`）、不含审计日志、不做真正分页（固定上限 + `has_more` 布尔，不是游标/offset）。机制上复用既有"共用路由 + 身份自动过滤"模式，不是独立的 admin-only 端点：`ctx.tenant_id()` 直接传给新增的 `SessionRepository::list_active(tenant_id, limit)`，`None`（admin）= 无过滤看全局，`Some(id)`（tenant）= 只看自己——与 owner_ref 推导同一套 `ctx.tenant_id()` passthrough，没有另开一条 admin 专用分支。配额快照的 `active_sessions` 计数刻意从这条新 SQL 查询取真实值，**没有**去统一 `open()` 准入路径当前用的内存计数器（`tenant_sessions`）——那个计数器在 daemon 重启后会漂移，是已知行为；统一两者是本次范围之外的下一步，明确搁置。实现：`crates/protocol/session/src/control.rs`（`SessionSummary`/`TenantQuotaSnapshot`/`SessionListResponse` 三个 DTO）+ `crates/core/src/application.rs`（`SessionRepository` trait 新增 `list_active` 必需方法，全仓库 11 处实现同步补齐；`SessionApplication::list_sessions` 复用既有 `audit_log` 包装）+ `crates/core/src/sqlite_repository.rs`（`list_active` 用 `status IN (...)` + `tenant_id = ?1 OR ?1 IS NULL` 两条查询）+ `apps/server/src/httpserver/session.rs`（`GET /api/v1/sessions` 路由 + handler，复用 `Extension<SecurityContext>` 模式）。
>
> 2026-08 四度更新（重启配额恢复）：配额快照与准入计数现均以 SQLite 中的 active session 聚合为事实来源。server 绑定 listener 前调用 `SessionApplication::restore_tenant_session_counts()` 恢复 `tenant_sessions`；聚合只统计 `opening/idle/running/paused`，排除 `failed/closed` 和 admin session。恢复失败时 fail-closed 拒绝启动，避免 daemon 重启将 `max_sessions` 使用量清零；open/load/fork 均计入，close 和失败回滚均释放。
>
> 2026-08 三度更新（管控面管理 API：新建/改配额/删除租户）：本节开头"现在建即是过度工程"这句话此刻已过期——一个真实用户需要它，因此本节末句提到的"数据库与管理 API"现已落地**管理 API 半边**（新建/改配额/删除，共 3 条路由：`POST/PATCH/DELETE /api/v1/admin/tenants[...]`，[http_api.md](./http_api.md) §3"租户管理（管控面，admin-only）"），**数据库那半边刻意没做**——持久化仍是直接改写 `tenants.toml`（原子写：临时文件 + rename）并复用既有 `SIGHUP` 重载同一份代码路径热更新 `TokenTable`，不是新开一张 SQLite 租户表；用户明确选择了这个方向而非数据库化。四项设计决策：(1) 持久化直改 `tenants.toml`，不是数据库；(2) 删除一个仍有活跃会话（`opening/idle/running/paused`）的租户直接拒绝、返回 409，**不**强制关闭会话——把"先清空会话"的责任留给操作者，而不是让一次 admin 调用产生大范围连锁副作用；(3) token 由服务端生成（`xgt_` 前缀 + 40 位随机字母数字），仅在创建响应中明文出现一次，此后任何接口都不再回显，遗失只能删除重建；(4) PATCH 是**部分字段更新**——请求体缺席的字段维持原值，标准的"双层 Option" serde 惯用法（`crates/protocol/session/src/tenant_admin.rs::deserialize_present_field`）区分"字段缺席"与"字段为 null"。三条路由只在服务端启动时已加载真实 `tenants.toml` 时才存在（`TenantAdminState` 只在 `main.rs` 的 `token_table.is_some()` 分支下才被构造并合并进路由——dev 模式下这三条路由整体不挂载，直接 404，不是 handler 内部分支判断"鉴权是否配置"）。删除同时保留了 `SIGHUP` reload 路径既有的"reload 产出空表永不应用"不变式的**主动**版本：若删除会导致 `tenants.toml` 归零凭证（无 `[admin]` 也无其余 `[[tenant]]`），同样在写盘前拒绝，返回 409。实现：`crates/protocol/session/src/tenant_admin.rs`（`TenantCreateRequest/Response`、`TenantPatchRequest/Response`、`TenantDeleteResponse` 五个 DTO）+ `crates/protocol/session/src/error.rs`（新增 `SessionWireError::TenantNotFound { tenant_id }` → 404，与既有按 `runtime_id` 的 `NotFound` 分属不同标识符空间）+ `apps/server/src/httpserver/tenant_config.rs`（新增 `write_tenants_file_atomically`/`read_tenants_file_raw`/`into_entries` 写路径，此前该文件只读）+ `apps/server/src/httpserver/admin_tenants.rs`（三个 handler + `TenantAdminState`，写路径持一把 `tokio::sync::Mutex<()>` 序列化并发 admin 写请求，防 read-modify-write 丢更新）+ `apps/server/src/httpserver/router.rs`（`create_router` 的 `tenant_admin: Option<TenantAdminState>` 参数，`Some` 时合并挂载）+ `apps/server/src/main.rs`（有 `tenants.toml` 时构造并传入 `TenantAdminState`）。测试：`admin_tenants.rs` 10 个用例（新建成功且 token 仅返一次、重复 tenant_id 409、空 tenant_id 400、PATCH 只改请求中出现的字段、PATCH 用显式 null 清空配额、PATCH 目标不存在 404、删除成功且 token 表随之失效、删除有活跃会话的租户 409、删除目标不存在 404、删除会清零凭证时 409）+ `router.rs` 2 个组合测试（`tenant_admin: None` 时路由结构性不存在、`Some` 时端到端可达）。
